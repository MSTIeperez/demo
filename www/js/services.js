...
.factory('Auth', function ($http, $cookieStore, $state, $parse) {
   var _user = localStorage.user ? $.parseJSON(localStorage.user) : null; //$cookieStore.get('starter.user');
...
.factory('Feeds_all', function($http, Auth, $q, $state, $location) {
  ...
  return {
    all: function(id_load,page,status,search,my_feeds, my_follow, fav,  subject_id, origin_id, theme_id, comision_id, feed_in) {
			var deferred = $q.defer();
            var promise = deferred.promise;
		$http.post(url+'/api/feeds/get_all',{'id_load':id_load,'page':page,'status':-1,'my_feeds':my_feeds,'my_follow':my_follow,'fav':fav, 'subject_id':subject_id, 'origin_id':origin_id, 'theme_id':theme_id, 'comision_id': comision_id, 'feed_in':feed_in})
...
.factory('Feeds', function($http, Auth, $q, $state, $location) {
  ...
  return {
    all: function(id_load,status,my_feeds, my_follow, fav,  subject_id, origin_id, theme_id, comision_id, feed_in) {
			var deferred = $q.defer();
            var promise = deferred.promise;
		$http.post(url+'/api/my-feeds/get_all',{ 'id_load':id_load,'status':-1,'my_feeds':my_feeds,'my_follow':my_follow,'fav':fav, 'subject_id':subject_id, 'origin_id':origin_id, 'theme_id':theme_id, 'comision_id': comision_id, 'feed_in':feed_in})
				.success(function(data){
				console.log(data);
				feeds=data.feeds;
				deferred.resolve(data);
...
.factory('Follow', function($http, Auth, $q, $state, $location) {
  ...
  return {
    all: function(id_load,status,my_feeds, my_follow, fav,  subject_id, origin_id, theme_id, comision_id, feed_in) {
			var deferred = $q.defer();
            var promise = deferred.promise;
		$http.post(url+'/api/follow/get_all',{'id_load':id_load,'status':-1,'my_feeds':my_feeds,'my_follow':my_follow,'fav':fav, 'subject_id':subject_id, 'origin_id':origin_id, 'theme_id':theme_id, 'comision_id': comision_id, 'feed_in':feed_in})
				.success(function(data){
				console.log(data);
				feeds=data.feeds;
				deferred.resolve(data);
